...
mod tests {
    ...
    #[test] fn synthetic_worst_case_soak_is_deterministic_and_conservative() {
        fn make_world() -> SimulationWorld {
            let mut world = SimulationWorld::new(64, 48);
            for y in 0..48 { for x in 0..64 { let pos = CellPos { x, y }; world.inject(pos, FluidId::Water, 500); world.inject(pos, FluidId::Lava, 500); world.inject(pos, FluidId::ToxicSlurry, 500); world.inject(pos, FluidId::Steam, 500); } }
            world
        }
        let mut first = make_world(); let mut second = make_world();
        for _ in 0..10_000 { first.tick(); second.tick(); }
