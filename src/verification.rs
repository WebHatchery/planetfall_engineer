...
pub const LAB_BAYS: [FluidBay; 9] = [
    FluidBay {
        name: "north_west_water",
        fluid: Some(FluidId::Water),
        origin: CellPos { x: 2, y: 2 },
    },
    FluidBay {
        name: "north_brine_reserved",
        fluid: None,
        origin: CellPos { x: 27, y: 2 },
    },
    FluidBay {
        name: "north_east_lava",
        fluid: Some(FluidId::Lava),
        origin: CellPos { x: 52, y: 2 },
    },
    FluidBay {
        name: "west_north_steam",
        fluid: Some(FluidId::Steam),
        origin: CellPos { x: 2, y: 20 },
    },
    FluidBay {
        name: "east_north_cryofluid_reserved",
        fluid: None,
        origin: CellPos { x: 52, y: 20 },
    },
    FluidBay {
        name: "west_south_acid_reserved",
        fluid: None,
        origin: CellPos { x: 2, y: 38 },
    },
    FluidBay {
        name: "east_south_slurry",
        fluid: Some(FluidId::ToxicSlurry),
        origin: CellPos { x: 52, y: 38 },
    },
    FluidBay {
        name: "south_west_nutrient_reserved",
        fluid: None,
        origin: CellPos { x: 20, y: 38 },
    },
    FluidBay {
        name: "south_east_aether_reserved",
        fluid: None,
        origin: CellPos { x: 38, y: 38 },
    },
];
...
impl FluidsLab {
    ...
    pub fn automatic_scenario(&mut self) -> LabReport {
        ...
        for _ in 0..300 {
            self.world.tick();
            reaction_events += self
                .world
                .events
                .iter()
                .filter(|event| {
                    matches!(event, crate::simulation::SimEvent::MaterialReacted { .. })
                })
                .count();
        }
        let reserved_bays_empty = LAB_BAYS
            .iter()
        ...
        LabReport {
            ...
            state_hash: state_hash(&self.world),
            active_bays: LAB_BAYS.iter().filter(|bay| bay.fluid.is_some()).count(),
            reserved_bays_empty,
            reaction_events,
            mass_balance_ok: self.world.mass_balance_error() == 0,
            passed: reserved_bays_empty
                && self.world.tick == 300
                && self.world.ledger.injected > 0
                && self.world.mass_balance_error() == 0,
        }
    }
}

impl Default for FluidsLab {
    fn default() -> Self {
        Self::new()
    }
}
