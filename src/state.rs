...
impl WorldState {
    ...
    pub fn new(width: usize, height: usize) -> Self {
        let cells = (0..height)
            .flat_map(|y| {
                (0..width).map(move |x| {
                    let ridge = ((x as i16 - width as i16 / 2).abs()
                        + (y as i16 - height as i16 / 2).abs())
                        * 125;
                    CellState {
                        height_hu: 1000 + ridge,
                        sealed: x == 0 || y == 0 || x + 1 == width || y + 1 == height,
                    }
                })
            })
            .collect();
...
impl GameSession {
    pub fn new(config: &GameConfig) -> Self {
        ...
        mission.start();
        Self {
            world: WorldState::new(config.world_width, config.world_height),
            simulation: SimulationWorld::new(config.world_width as u16, config.world_height as u16),
            mission,
            campaign: CampaignProgress::default(),
            tick: 0,
            selected: CellPos {
                x: (config.world_width / 2) as u16,
                y: (config.world_height / 2) as u16,
            },
            time_control: TimeControl::Paused,
            accumulator: 0.0,
        }
    }
...
pub fn save_session(session: &GameSession, config: &GameConfig) -> Result<(), String> {
    save_to_slot_with_version(
        &config.game_name,
        &config.save_slot,
        &session.to_save(&config.version),
        &config.version,
    )
}
...
pub fn load_session(config: &GameConfig) -> Result<GameSession, String> {
    let save: SaveData = load_from_slot_with_migration(
        ...
        &config.save_slot,
        &config.version,
        |_, value| migrate_save_value(value, config),
    )?;
...
fn migrate_save_value(value: Value, config: &GameConfig) -> Result<SaveData, String> {
    ...
    if save.schema_version != SAVE_SCHEMA_VERSION {
        return Err(format!(
            "Save schema {} is unsupported; expected {}",
            save.schema_version, SAVE_SCHEMA_VERSION
        ));
    }
    if save.version != config.version {
        return Err(format!(
            "Save content version {} does not match installed content {}",
            save.version, config.version
        ));
    }
    if save.world.width as usize != config.world_width
        || save.world.height as usize != config.world_height
    {
        return Err("Save world dimensions do not match content".into());
    }
    if save.simulation.width != save.world.width || save.simulation.height != save.world.height {
        return Err("Save simulation dimensions do not match world dimensions".into());
    }
    Ok(save)
}
