...
impl Game {
    pub async fn new() -> Self {
        let data = GameData::load().unwrap_or_else(|err| {
            panic!("Planetfall Engineer embedded data failed to load: {}", err);
        });
        ...
        notifications.info(format!(
            "Survey systems online; {} manifest textures loaded",
            loaded_assets
        ));
    ...
    fn apply_action(&mut self, action: UiAction) {
        match action {
            UiAction::NewGame => {
                ...
                self.notifications
                    .info("Started a fresh Ashfall Basin survey");
            }
    ...
    fn delete_save(&mut self) {
        match delete_slot(&self.data.config.game_name, &self.data.config.save_slot) {
            Ok(()) => {
                self.notifications
                    .info("Deleted the saved engineering plan");
                self.refresh_save_state();
            }
