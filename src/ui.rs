...
fn draw_header(data: &GameData, session: &GameSession) {
    ...
    draw_badge(
        ...
        &format!("Cycle {}", session.player.turn),
...
fn draw_world_panel(ctx: &UiContext<'_>, mouse: Vec2) {
    ...
    draw_surface_with_title(
        rect,
        Some("Ashfall Basin | Survey Grid"),
        &style,
    ...
    if grid_rect.contains_point(mouse) {
        draw_tooltip(
            "Arrow keys move the survey marker. Right mouse drag and +/- adjust the field camera.",
            mouse,
        );
    }
}
...
fn draw_footer() {
    ...
    draw_text_block(
        "Phase 0 foundation: survey grid, terrain selection, engineering actions, camera controls, mission saves, notifications, and data-driven content.\n\nMission brief: establish a safe route from the upland meltwater source to the dormant restoration basin. Lava containment and fluid simulation arrive in the next foundation pass.",
        rect.x + 18.0,
        rect.y + 14.0,
        rect.w - 36.0,
        rect.h - 20.0,
