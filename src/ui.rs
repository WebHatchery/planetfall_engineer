...
use crate::{data::GameData, mission::MissionPhase, simulation::FluidId, state::{GameSession, TimeControl}};
...
pub fn draw_hud(ctx: UiContext<'_>) {
    ...
    draw_ui_text_ex(&format!("L0{} // {} // {}", ctx.session.mission.id.sequence(), ctx.session.mission.id.name().to_uppercase(), phase_name(ctx.session.mission.phase)), 30.0, 64.0, TextStyle::new(13.0, Color::new(0.48, 0.62, 0.72, 1.0)).params());
    ...
    draw_ui_text_ex("Click select   WASD pan   Q/E rotate   +/- zoom   Arrows survey   Space pause   1/2/4 speed   B/P/O/F queue   Enter commit   Backspace cancel   J/K/H gate   F5/F6/F9/F12 save/checkpoint/load/reset", 44.0, 682.0, TextStyle::new(13.0, Color::new(0.5, 0.58, 0.64, 1.0)).params());
